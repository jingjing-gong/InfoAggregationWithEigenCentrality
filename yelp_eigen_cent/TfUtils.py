...
def selu(feature, lamb, alpha):
    out = lamb * tf.where(feature > 0, feature, alpha*tf.exp(feature) - alpha)
    return out

def iterEigenCentrality(matrix, max_iter):
    '''
    ...
    def body(time, eigenvec):
        '''

        :param eigenvec: shape(b_sz, dim, 1)
        :return out:     shape(b_sz, dim, 1)
        '''
        eig_new = tf.matmul(matrix, eigenvec)
        eig_out = eig_new / (tf.reduce_sum(tf.abs(eig_new), axis=1, keep_dims=True) + 1e-30)
        return time+1, eig_out

    zero_step = tf.constant(0, dtype=tf.int32)
    init_eig = tf.ones([b_sz, dim, 1], dtype=matrix.dtype)
    init_eig = init_eig / tf.reduce_sum(init_eig, axis=1, keep_dims=True)

    _, out = tf.while_loop(lambda i, _: tf.less(i, max_iter), body=body, loop_vars=[zero_step, init_eig])
    out = tf.squeeze(out, axis=[2])   # shape(b_sz, dim)
    # out = out / (tf.reduce_sum(out, axis=1, keepdims=True) + 1e-30)
    return out


def maskedLN(inp, mask):
    '''

    :param inp: shape(b_sz, xlen, xlen)
    :param mask:  shape(b_sz, xlen, xlen), dtype=bool
    :return out:    shape(b_sz, xlen, xlen)
    '''
    epsilon = 1e-30
    masked_inp = tf.where(mask, inp, tf.zeros_like(inp))    # shape(b_sz, xlen, xlen)
    inp_sum = tf.reduce_sum(masked_inp, axis=[1, 2], keepdims=True)   # shape(b_sz, 1, 1)
    cnt_sum = tf.reduce_sum(tf.cast(mask, dtype=tf.float32), axis=[1, 2], keepdims=True) # shape(b_sz, 1, 1)
    inp_mean = inp_sum / (cnt_sum + epsilon)    # shape(b_sz, 1, 1)

    tmp = tf.where(mask, (inp - inp_mean)**2, tf.zeros_like(inp))   # shape(b_sz, xlen, xlen)
    tmp_sum = tf.reduce_sum(tmp, axis=[1, 2], keepdims=True)    # shape(b_sz, 1, 1)
    moment = tf.sqrt(tmp_sum/(cnt_sum+epsilon) + epsilon)                 # shape(b_sz, 1, 1)

    out = tf.where(mask, (inp-inp_mean)/moment, tf.zeros_like(inp))     # shape(b_sz, xlen, xlen)
    return out
